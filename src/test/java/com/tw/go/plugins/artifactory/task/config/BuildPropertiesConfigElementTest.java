...
public class BuildPropertiesConfigElementTest {
    ...
    public void shouldValidateKeyValuePairsSeparatedByNewline() {
        ASSERT.that(buildProperties.validate("a=b")).isAbsent();
        ASSERT.that(buildProperties.validate("a=b\nc=d")).isAbsent();
        ASSERT.that(buildProperties.validate("a=b\n\n\nc=d\n\n")).isAbsent();
    }

    @Test
    public void shouldNotValidateIfNotCorrectlyFormatted() {
        ASSERT.that(buildProperties.validate("a=b=c")).hasValue(new ValidationError("properties", "Invalid properties format"));
    }
